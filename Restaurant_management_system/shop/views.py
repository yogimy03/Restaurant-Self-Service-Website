...
def index(request):
    ...
    G4 = {'allProds': allProds}
    return render(request, 'shop/index.html', G4)
...
def search(request):
    ...
    for cat in cats:
        ...
        if len(prod) != 0:
            allProds.append([prod, range(1, nSlides), nSlides])
    G4 = {'allProds': allProds, "msg": ""}
    if len(allProds) == 0 or len(query) < 3:
        G4 = {'msg': "No item available. Please make sure to enter relevant search query"}
    return render(request, 'shop/search.html', G4)
...
def checkout(request):
    if request.method == "POST":
        ...
        if 'onlinePay' in request.POST:
            ...
            G4_dict['CHECKSUMHASH'] = Checksum.generate_checksum(G4_dict, MERCHANT_KEY)
            return render(request, 'shop/paytm.html', {'G4_dict': G4_dict})
